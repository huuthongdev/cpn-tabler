...
export default function CpnCheckBox(props) {
    const { label, value, onChange, type } = props;
    return <div
        onClick={() => {
            if (onChange) return onChange(!value);
            console.log('(onChange) function must be provided.');
        }}
        className={`cpn-table--cpn-check-box ${value ? 'checked' : ''} ${type === 'radio' ? 'radio' : ''}`}
    >
        <div className="icon">
